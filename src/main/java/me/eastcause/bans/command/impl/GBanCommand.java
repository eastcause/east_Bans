...
public class GBanCommand extends Cmd {
    public GBanCommand() {
        super("gban");
        argument = (sender, args) -> {
            if(args.length == 0){
                return Utils.sendMessage(sender, "&cUsage: &6/gban <nick> <time> <reason> <server ? server=serverName>");
            }
            String name = args[0];
            UUID uuid = null;
            ...
            if(args.length == 1){
                Utils.sendMessage("&bPlayer: &6"+name+" &bwas &6permanently &bbanned by &6"+admin+" &bwith reason: &7"+reason+" &bon the &6proxy &bserver.", getPermission());
                return BanManager.banPlayer(uuid, name, reason, time, true, admin, true, "proxy");
            }
            ...
            if(proxy) {
                BanManager.banPlayer(uuid, name, reason, time, perm, admin, true, "proxy");
            }else{
                BanManager.banPlayer(uuid, name, reason, time, perm, admin, true, (servers.toArray(new String[0])));
            }
            Utils.sendMessage("&bPlayer: &6"+name+" &bwas &6"+(perm ? "permanently" : "temporarily")+" &bbanned "+(perm ? "" : "for: &6" + TimeUtil.longToStringTime(time) + "")+"&bby &6"+admin+" &bwith reason: &7"+reason+" &bon the &6"+(proxy ? "proxy" : servers.toString().toLowerCase().replace("[", "").replace("]", ""))+" &bserver.", getPermission());
            return false;
        };
    }
